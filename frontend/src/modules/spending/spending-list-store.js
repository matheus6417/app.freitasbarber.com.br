...
import spendingListExporterFields from '@/modules/spending/spending-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!spendingListExporterFields || !spendingListExporterFields.length) {
          throw new Error('spendingListExporterFields is required');
        }
        ...
        const response = await SpendingService.list(
          filter,
          getters.orderBy,
          null,
          null,
